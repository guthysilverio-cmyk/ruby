...
-- ============================================================
-- RUBY ADDON: 7M PAID
-- Added as a separate tab; existing RUBY systems are untouched.
-- ============================================================
local R7M_State = {
    HitboxSize = 5,
    ...
    AkariDash = false,
    ScoreEffect = false,
    ScoreEffectName = "SupernovaScoreEffect",
    PlayerCard = false,
    ESP = false,
    ESPJump = false,
    AirMove = false,
    ...
    AkariButton = nil,
    ScoreConnection = nil,
    PlayerCardConnection1 = nil,
    PlayerCardConnection2 = nil,
    ESPObjects = {},
    ESPJumpObjects = {},
    ShiftConnections = {},
}

local function R7M_Notify(text)
    if t2.value38 then
        task.spawn(function()
            pcall(t2.value38, "7M PAID • " .. tostring(text))
        end)
    end
end

local function R7M_Service(name)
    ...
    return services and services:FindFirstChild(name)
end

local function R7M_RF(serviceName, remoteName, ...)
    local service = R7M_Service(serviceName)
    local rf = service and service:FindFirstChild("RF")
    local remote = rf and rf:FindFirstChild(remoteName)
    if not remote then
        R7M_Notify("Remote não encontrado: " .. serviceName .. "." .. remoteName)
        return false
    end
...
local function R7M_RE(serviceName, remoteName)
    local service = R7M_Service(serviceName)
    local re = service and service:FindFirstChild("RE")
    return re and re:FindFirstChild(remoteName)
end

local function R7M_Button(parent, name, callback)
    local frame = Instance.new("Frame")
    ...
    button.BackgroundColor3 = t2.value26.ThemeColor
    button.Text = name
...
local function R7M_RemoveHitboxes()
    if t2.value150 then
        t2.value150()
    end
end

local function R7M_SetHitboxColor(color)
    R7M_State.HitboxColor = color
end

-- Runs after RUBY's own hitbox renderer and only affects hitboxes while
-- the 7M control is enabled, leaving the original RUBY renderer intact.
R7M_State.HitboxColorConnection = t2.value3.RenderStepped:Connect(function()
    if not t2.value26.HitboxEnabled then return end
    for _, child in ipairs(t2.value6:GetChildren()) do
        if child.Name:match("^CLIENT_BALL_") then
            local ball = child:FindFirstChild("Ball.001")
            if ball and ball:IsA("BasePart") then
                ball.Color = R7M_State.HitboxColor
                ball.Size = Vector3.new(2, 2, 2) * R7M_State.HitboxSize
            end
        end
    end
end)

local function R7M_SetAntiLag(enabled)
    R7M_State.AntiLag = enabled
    pcall(function()
        if enabled then
            if t2.value7:FindFirstChild("BloomEffect") then t2.value7.BloomEffect.Enabled = false end
            if t2.value7:FindFirstChild("BlurEffect") then t2.value7.BlurEffect.Enabled = false end
            if t2.value7:FindFirstChild("SunRaysEffect") then t2.value7.SunRaysEffect.Enabled = false end
    ...
    R7M_Notify(enabled and "Anti-Lag ON" or "Anti-Lag OFF")
end

local function R7M_SetDesync(enabled)
    R7M_State.Desync = enabled
    if R7M_State.DesyncConnection then
        R7M_State.DesyncConnection:Disconnect()
        R7M_State.DesyncConnection = nil
    end
    if not enabled then
        R7M_Notify("Desync OFF")
        return
    end

    local doMove = R7M_RE("BallService", "DoMove")
    if not doMove then
        R7M_Notify("DoMove não encontrado")
        return
    end

    R7M_State.DesyncConnection = t2.value2.JumpRequest:Connect(function()
        if not R7M_State.Desync then return end
        task.wait(0.03)
        if firesignal then
            pcall(function()
    ...
    R7M_Notify("Desync ON")
end

local function R7M_SetAkari(enabled)
    R7M_State.AkariDash = enabled

    if not enabled then
        if R7M_State.AkariGui then
            R7M_State.AkariGui:Destroy()
            R7M_State.AkariGui = nil
            R7M_State.AkariButton = nil
        end
        R7M_Notify("Akari Dash OFF")
        return
    end

    if R7M_State.AkariGui then
        R7M_State.AkariGui.Enabled = true
        return
    end

    local gui = Instance.new("ScreenGui")
    gui.Name = "Ruby7MAkariDash"
    gui.ResetOnSpawn = false
    gui.Parent = PlayerGui
    R7M_State.AkariGui = gui

    local button = Instance.new("TextButton")
    button.Size = UDim2.new(0, 50, 0, 50)
    button.Position = UDim2.new(0.15, -25, 0.8, -25)
    ...
    R7M_State.AkariButton = button

    local dragging = false
    local startPos, startButton
    ...
    t2.value2.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            button.Position = UDim2.new(startButton.X.Scale, startButton.X.Offset + delta.X, startButton.Y.Scale, startButton.Y.Offset + delta.Y)
        end
    end)

    button.MouseButton1Click:Connect(function()
        if not R7M_State.AkariDash then return end
        local character = t2.value23.Character
        local root = character and character:FindFirstChild("HumanoidRootPart")
        if not root then return end
        local look = t2.value24.CFrame.LookVector
        local flat = Vector3.new(look.X, 0, look.Z)
        if flat.Magnitude <= 0 then return end
        flat = flat.Unit
        pcall(function()
            t2.value1:Create(root, TweenInfo.new(0.2, Enum.EasingStyle.Linear), {
                CFrame = CFrame.new(root.Position + flat * 10) * CFrame.Angles(0, math.rad(root.Orientation.Y), 0)
            }):Play()
        end)
    end)
    R7M_Notify("Akari Dash ON")
end

local function R7M_SetScoreEffect(enabled)
    R7M_State.ScoreEffect = enabled
    if R7M_State.ScoreConnection then
        R7M_State.ScoreConnection:Disconnect()
        R7M_State.ScoreConnection = nil
    end
    if not enabled then
        R7M_Notify("ScoreEffect OFF")
        return
    end

    local hitGround = R7M_RE("BallService", "HitGround")
    if not hitGround then
        R7M_Notify("HitGround não encontrado")
        return
    end

    local playerName = t2.value23.Name
    local effectName = R7M_State.ScoreEffectName

    local function fireEffect(position)
        if not R7M_State.ScoreEffect or not firesignal then return end
        local playerCharacter = workspace:FindFirstChild(playerName) or workspace:FindFirstChild("uwuwuwiiwuw6")
        pcall(function()
            firesignal(hitGround.OnClientEvent,
                position, true, false, 5, effectName, playerCharacter,
    ...
    local function inspect(obj)
        ...
        if obj.Name == "HitIndicator" and obj.Color == Color3.fromRGB(0, 255, 0) then
            fireEffect(obj.Position)
        end
    end

    R7M_State.ScoreConnection = workspace.DescendantAdded:Connect(inspect)
    for _, obj in ipairs(workspace:GetDescendants()) do
        inspect(obj)
    end
    R7M_Notify("ScoreEffect ON")
end

local function R7M_SetPlayerCard(enabled)
    R7M_State.PlayerCard = enabled
    if R7M_State.PlayerCardConnection1 then R7M_State.PlayerCardConnection1:Disconnect() end
    if R7M_State.PlayerCardConnection2 then R7M_State.PlayerCardConnection2:Disconnect() end
    R7M_State.PlayerCardConnection1 = nil
    R7M_State.PlayerCardConnection2 = nil

    if not enabled then
        R7M_Notify("PlayerCard OFF")
        return
    end

    local card = R7M_RE("PlayerCardService", "OnFlashCard")
    if not card then
        R7M_Notify("OnFlashCard não encontrado")
        return
    end

    local locked = false
    local function showCard()
        if locked or not R7M_State.PlayerCard or not firesignal then return end
        locked = true
        pcall(function()
            firesignal(card.OnClientEvent, {
                PlayerCardItemId = "UltimateChampionPlayerCard",
                Context = {Custom = {}, PlacementId = "ScoredPoint"},
                Player = t2.value23
            })
        end)
    end

    R7M_State.PlayerCardConnection1 = PlayerGui.DescendantAdded:Connect(function(descendant)
        if descendant.Name == "FlashPlayerCardFrame" and R7M_State.PlayerCard then
            descendant:Destroy()
            task.wait(0.1)
            showCard()
        end
    end)
    R7M_State.PlayerCardConnection2 = PlayerGui.DescendantRemoving:Connect(function(descendant)
        if descendant.Name == "FlashPlayerCardFrame" then
            locked = false
        end
    end)
    R7M_Notify("PlayerCard ON")
end

local function R7M_ClearESP()
    for player, obj in pairs(R7M_State.ESPObjects) do
        if obj and obj.Parent then obj:Destroy() end
        R7M_State.ESPObjects[player] = nil
    end
end

local function R7M_AddESP(player)
    if player == t2.value23 or not player.Character then return end
    if R7M_State.ESPObjects[player] and R7M_State.ESPObjects[player].Parent then return end
    ...
    local labels = {
        {player:GetAttribute("User_InputType") or "N/A", Color3.fromRGB(255,255,255)},
        {player:GetAttribute("Gameplay_Style") or "N/A", Color3.fromRGB(255,200,0)},
        {player:GetAttribute("Gameplay_Ability") or "N/A", Color3.fromRGB(0,200,255)},
        {player:GetAttribute("User_Level") or "N/A", Color3.fromRGB(0,255,100)}
    }
    for i, data in ipairs(labels) do
        ...
        label.Text = tostring(data[1])
        label.TextColor3 = data[2]
        label.TextScaled = true
        label.Font = Enum.Font.GothamBold
        label.Parent = frame
    end
    R7M_State.ESPObjects[player] = gui
end

local function R7M_SetESP(enabled)
    R7M_State.ESP = enabled
    if not enabled then
        R7M_ClearESP()
        R7M_Notify("ESP OFF")
        return
...
t2.value4.PlayerAdded:Connect(function(player)
    if R7M_State.ESP then
        task.spawn(function()
...
local function R7M_ClearESPJump()
    for player, obj in pairs(R7M_State.ESPJumpObjects) do
        if obj and obj.Parent then obj:Destroy() end
        R7M_State.ESPJumpObjects[player] = nil
    end
end

local function R7M_SetESPJump(enabled)
    R7M_State.ESPJump = enabled
    if not enabled then
        R7M_ClearESPJump()
        R7M_Notify("ESP Jump OFF")
        return
    end

    for _, player in ipairs(t2.value4:GetPlayers()) do
        if player ~= t2.value23 then
            local function bind(character)
                local humanoid = character:WaitForChild("Humanoid", 3)
                if not humanoid then return end
                humanoid.StateChanged:Connect(function(_, state)
                    if not R7M_State.ESPJump or not player.Character then return end
                    if state == Enum.HumanoidStateType.Jumping or state == Enum.HumanoidStateType.Freefall then
                        if not R7M_State.ESPJumpObjects[player] then
                            local h = Instance.new("Highlight")
                            h.Name = "Ruby7MJumpESP"
                            ...
                            R7M_State.ESPJumpObjects[player] = h
                        end
                    elseif state == Enum.HumanoidStateType.Landed then
                        local h = R7M_State.ESPJumpObjects[player]
                        if h then h:Destroy(); R7M_State.ESPJumpObjects[player] = nil end
                    end
                end)
            end
            if player.Character then task.spawn(bind, player.Character) end
            player.CharacterAdded:Connect(function(c)
                if R7M_State.ESPJump then task.spawn(bind, c) end
            end)
        end
    end
    R7M_Notify("ESP Jump ON")
end

local function R7M_SetAirMove(enabled)
    R7M_State.AirMove = enabled
    if R7M_State.AirConnection then
        R7M_State.AirConnection:Disconnect()
        R7M_State.AirConnection = nil
    end
    if not enabled then
        local character = t2.value23.Character
        local root = character and character:FindFirstChild("HumanoidRootPart")
        if root then root.Velocity = Vector3.zero end
        R7M_Notify("Air Movement OFF")
        return
    end

    R7M_State.AirConnection = t2.value3.RenderStepped:Connect(function()
        if not R7M_State.AirMove then return end
        local character = t2.value23.Character
        local humanoid = character and character:FindFirstChildOfClass("Humanoid")
        local root = character and character:FindFirstChild("HumanoidRootPart")
        if humanoid and root then
            root.Velocity = humanoid.MoveDirection * R7M_State.AirSpeed
        end
    end)
    R7M_Notify("Air Movement ON")
end

local function R7M_SetAutoShiftLock(enabled)
    R7M_State.AutoShiftLock = enabled
    for _, c in ipairs(R7M_State.ShiftConnections) do pcall(function() c:Disconnect() end) end
    R7M_State.ShiftConnections = {}
    if not enabled then
        R7M_Notify("Auto Shift Lock OFF")
        return
    end
    ...
    local function bind(character)
        ...
        table.insert(R7M_State.ShiftConnections, humanoid:GetPropertyChangedSignal("Jump"):Connect(function()
            if humanoid.Jump and R7M_State.AutoShiftLock then
                task.defer(function()
                    task.wait(0.03)
                    local look = t2.value24.CFrame.LookVector
                    local flat = Vector3.new(look.X, 0, look.Z)
    ...
    if t2.value23.Character then bind(t2.value23.Character) end
    table.insert(R7M_State.ShiftConnections, t2.value23.CharacterAdded:Connect(function(c)
        task.wait(0.2)
        if R7M_State.AutoShiftLock then bind(c) end
    end))
    R7M_Notify("Auto Shift Lock " .. (enabled and "ON" or "OFF"))
end

-- New 7M tab. Existing RUBY tabs and systems are not replaced.
local R7M_Tab, R7M_Panel = v68("7M PAID")

v67(R7M_Panel, "Hitbox", "r7mHitbox", function(on)
    t2.value26.HitboxEnabled = on
    if not on then R7M_RemoveHitboxes() end
    if t2.value56 then t2.value56() end
end)

t1.value2(R7M_Panel, "Hitbox Size", 0, 20, R7M_State.HitboxSize, "r7mHitboxSize", function(value)
    R7M_State.HitboxSize = value
    t2.value26.HitboxSize = value
end)

v69(R7M_Panel, "Hitbox Color", {"Green","Blue","Red","Yellow","Purple","Orange","Pink","White"}, "r7mHitboxColor", function(value)
    local colors = {
        Green=Color3.fromRGB(0,255,0), Blue=Color3.fromRGB(0,0,255),
        Red=Color3.fromRGB(255,0,0), Yellow=Color3.fromRGB(255,255,0),
    ...
    R7M_SetHitboxColor(colors[value] or colors.Green)
end)

R7M_Button(R7M_Panel, "Remove Hitboxes", function()
    R7M_RemoveHitboxes()
end)

R7M_Button(R7M_Panel, "Show / Hide Stop Ball", function()
    if not t2.value39 then return end
    t2.value39.Visible = not t2.value39.Visible
end)

R7M_Button(R7M_Panel, "Show / Hide Aim", function()
    if not t2.value107 then return end
    t2.value107.Visible = not t2.value107.Visible
end)

v67(R7M_Panel, "Anti-Lag", "r7mAntiLag", R7M_SetAntiLag)
v67(R7M_Panel, "Desync", "r7mDesync", R7M_SetDesync)
v67(R7M_Panel, "Akari Dash", "r7mAkariDash", R7M_SetAkari)
v67(R7M_Panel, "ScoreEffect Changer", "r7mScoreEffect", R7M_SetScoreEffect)

v69(R7M_Panel, "Effect Select", {
    "SupernovaScoreEffect",
    "MoonlightBeamScoreEffect",
    "VioletBeamScoreEffect",
    "Season10ProScoreEffect"
}, "r7mEffectSelect", function(value)
    R7M_State.ScoreEffectName = value
    if R7M_State.ScoreEffect then
        R7M_SetScoreEffect(false)
        R7M_SetScoreEffect(true)
    end
end)

v67(R7M_Panel, "PlayerCard Changer", "r7mPlayerCard", R7M_SetPlayerCard)
v67(R7M_Panel, "ESP", "r7mESP", R7M_SetESP)
v67(R7M_Panel, "ESP (Jump)", "r7mESPJump", R7M_SetESPJump)

t1.value2(R7M_Panel, "Dive Speed", 0, 3, 0.95, "r7mDiveSpeed", function(value)
    t2.value23:SetAttribute("Multiplier_DiveSpeed", value)
end)
t1.value2(R7M_Panel, "Jump Power", 0, 3, 1.1, "r7mJumpPower", function(value)
    t2.value23:SetAttribute("Multiplier_JumpPower", value)
end)
t1.value2(R7M_Panel, "Speed", 0, 3, 0.85, "r7mSpeed", function(value)
    t2.value23:SetAttribute("Multiplier_Speed", value)
end)

v67(R7M_Panel, "Air Movement", "r7mAirMove", R7M_SetAirMove)
t1.value2(R7M_Panel, "Air Movement Speed", 0, 100, 16, "r7mAirSpeed", function(value)
    R7M_State.AirSpeed = value
end)
v67(R7M_Panel, "Auto Shift Lock", "r7mAutoShiftLock", R7M_SetAutoShiftLock)

t1.value2(R7M_Panel, "POV", 1, 120, 70, "r7mPOV", function(value)
    t2.value24.FieldOfView = value
end)
R7M_Button(R7M_Panel, "Rejoin", function()
    pcall(function()
        game:GetService("TeleportService"):Teleport(game.PlaceId, t2.value23)
    end)
end)

R7M_Button(R7M_Panel, "Buy Emote", function()
    if R7M_RF("PackService", "Open", "Emote1") then R7M_Notify("Emote purchased!") end
end)
R7M_Button(R7M_Panel, "Buy Effect", function()
    if R7M_RF("PackService", "Open", "ScoreEffect1") then R7M_Notify("Effect purchased!") end
end)
R7M_Button(R7M_Panel, "Buy Extreme Ball", function()
    if R7M_RF("PackService", "Open", "Extreme") then R7M_Notify("Extreme ball purchased!") end
end)
R7M_Button(R7M_Panel, "Buy Medium Ball", function()
    if R7M_RF("PackService", "Open", "Medium") then R7M_Notify("Medium ball purchased!") end
end)
R7M_Button(R7M_Panel, "Buy Basic Ball", function()
    if R7M_RF("PackService", "Open", "Basic") then R7M_Notify("Basic ball purchased!") end
end)

R7M_Button(R7M_Panel, "Claim Level Rewards", function()
    if R7M_RF("LevelService", "ClaimLevelRewards") then R7M_Notify("Level rewards claimed!") end
end)
R7M_Button(R7M_Panel, "Claim Quest Rewards", function()
    if R7M_RF("QuestService", "ClaimAll", true) then R7M_Notify("Quest rewards claimed!") end
end)

R7M_Button(R7M_Panel, "Redeem All Codes", function()
    local ok, result = pcall(function()
        return game:HttpGet("https://beebom.com/haikyuu-legends-codes/")
    end)
    if not ok or not result then
        R7M_Notify("Não foi possível obter os códigos")
        return
    end
    local codes, seen = {}, {}
    for match in string.gmatch(result, "[^\n]+") do
        if string.find(match, "(NEW)") then
            local code = string.match(match, "(%u+[%u%d_]+)")
            if code and not seen[code] then
                seen[code] = true
                table.insert(codes, code)
            end
        end
        if #codes >= 3 then break end
    end
    local redeemed = 0
    for _, code in ipairs(codes) do
        if R7M_RF("CodeService", "Redeem", code) then redeemed += 1 end
        task.wait(0.01)
    end
    R7M_Notify(redeemed > 0 and ("Codes redeemed: " .. redeemed) or "No codes found")
end)
...
for _, mode in ipairs(gameModes) do
    local label, id = mode[1], mode[2]
    R7M_Button(R7M_Panel, label, function()
        if R7M_RF("PartyService", "RequestTeleport", id) then
            R7M_Notify("Joining " .. label .. "...")
        end
    end)
end

R7M_Button(R7M_Panel, "Save RUBY Config", function()
    if t2.value67() then R7M_Notify("Config saved!") end
end)
R7M_Button(R7M_Panel, "Load RUBY Config", function()
    local config = t2.value68()
    if config then
        t2.value69(config)
        t2.value70()
        R7M_Notify("Config loaded!")
    else
        R7M_Notify("No config found")
    end
end)
R7M_Button(R7M_Panel, "Panic", function()
    t2.value150()
    R7M_SetDesync(false)
    R7M_SetAkari(false)
    R7M_SetScoreEffect(false)
    R7M_SetPlayerCard(false)
    R7M_SetESP(false)
    R7M_SetESPJump(false)
    R7M_SetAirMove(false)
    R7M_SetAutoShiftLock(false)
    R7M_Notify("All 7M features disabled")
end)
R7M_Button(R7M_Panel, "Unload", function()
    R7M_SetDesync(false)
    R7M_SetAkari(false)
    R7M_SetScoreEffect(false)
    R7M_SetPlayerCard(false)
...
R7M_Tab.MouseButton1Click:Connect(function()
    v144(R7M_Tab, R7M_Panel)
end)
