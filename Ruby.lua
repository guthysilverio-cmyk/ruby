...
function t2.value174()
    -- Aplica o tema escolhido em toda a interface, sem alterar o fundo escuro.
    -- Os presets de cor ficam de fora para continuarem mostrando suas próprias cores.
    ...
    local presetContainer = Frame14

    local function isInsidePreset(obj)
        if not presetContainer then
            return false
        end
        return obj == presetContainer or obj:IsDescendantOf(presetContainer)
    end

    local function apply(obj)
        if not obj or not obj:IsA("GuiObject") or isInsidePreset(obj) then
            return
        end
        ...
        if obj:IsA("TextButton") or obj:IsA("ImageButton") or obj:IsA("TextBox") then
            ...
            if obj:IsA("TextButton") or obj:IsA("TextBox") then
                obj.TextColor3 = (theme == black) and Color3.fromRGB(220, 220, 220) or white
            end
        end
        ...
        if obj.Name == "SliderFill"
            ...
            or obj.Name == "ETERNALHUBThemeButton" then
            obj.BackgroundColor3 = theme
        end

        -- Elementos que representam seleção/estado ativo.
        if obj:IsA("Frame") and (obj.Name == "TabButton" or obj.Name == "Selected" or obj.Name == "Active") then
            obj.BackgroundColor3 = theme
        end
    end

    -- O container principal da interface.
    if t2.value104 then
    ...
    -- Elementos criados fora da árvore principal, mas usados pelo menu.
    for _, obj in ipairs({
        t2.value35, t2.value36, t2.value37, t2.value116,
        t2.value118, t2.value124, t2.value129, t2.value130,
        t2.value131, t2.value133, t2.value135, t2.value137,
        t2.value139, t2.value141, t2.value143, t2.value145,
        t2.value148, t2.value152, t2.value154, t2.value156,
        t2.value158, t2.value159, t2.value160
    }) do
        if obj then
            if obj:IsA("UIStroke") then
                obj.Color = theme
    ...
    -- O botão selecionado da aba atual sempre acompanha a cor escolhida.
    if t2.value125 then
        for _, obj in ipairs(t2.value125:GetChildren()) do
    ...
    -- Indicadores de seleção, sliders e toggles.
    if t2.value127 then
        for _, obj in ipairs(t2.value127:GetDescendants()) do
            if obj.Name == "SliderFill" or obj.Name == "ToggleButton" then
                obj.BackgroundColor3 = theme
    ...
    -- Mantém o botão de salvar e o link com a cor atual.
    if t2.value35 then
        t2.value35.BackgroundColor3 = theme
    end
    if t2.value36 then
        t2.value36.BackgroundColor3 = theme
        t2.value36.TextColor3 = (theme == black) and Color3.fromRGB(220, 220, 220) or white
    end
    if t2.value37 then
        t2.value37.Color = theme
    end

    -- Texto de destaque da interface.
    if t2.value124 then
        t2.value124.TextColor3 = (theme == white) and Color3.fromRGB(220, 220, 220) or theme
    end
    if t2.value130 then
        t2.value130.TextColor3 = (theme == white) and Color3.fromRGB(220, 220, 220) or theme
    end

    -- Atualiza elementos externos já registrados pelo hub.
    if t2.value56 then
        pcall(t2.value56)
    end
    if t2.value58 then
